...
// POST /api/refunds - crear devolución (versión corregida)
router.post("/", verifyToken, async (req, res) => {
  const client = await pool.connect();
  try {
    const parsed = refundSchema.parse(req.body);
    const { sale_id, items, reason } = parsed;
    ...
    const refundRes = await client.query(
      `INSERT INTO refunds (sale_id, cash_session_id, user_id, reason, total_refunded, status)
       VALUES ($1::uuid, $2::integer, $3::uuid, $4, $5, 'completed') RETURNING id`,
      [
        sale_id,
    ...
    for (const item of refundItemsData) {
      ...
      // Movimiento de stock (sin refund_id)
      await client.query(
